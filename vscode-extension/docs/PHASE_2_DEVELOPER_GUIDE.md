...
describe("MyParticipant", () => {
  ...
  it("should resolve context when @mentioned", async () => {
    ...
    const result = await participant.resolveReferenceContext("@workspace", context);
...
### Enable Debug Output
```typescript
const outputChannel = vscode.window.createOutputChannel("VTCode Debug");
...
console.log("Registered commands:", allCommands.map(c => c.id));

const applicable = registry.getApplicableParticipants(context);
console.log("Applicable participants:", applicable.map(p => p.id));
```
