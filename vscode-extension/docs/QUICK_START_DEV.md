# VTCode VSCode Extension - Developer Quick Start

This guide is for contributors implementing the VTCode VSCode extension roadmap (Phases 1–2). It explains how to run, debug, and extend the extension using the existing architecture and the new docs in this repo.
...
- VTCode CLI installed and on PATH:
...
- Rust toolchain (for working on the core VTCode repo)
...
- `src/extension.ts`
  - Main entrypoint:
    ...
    - Integrates with VTCode CLI, MCP, and VS Code AI APIs.
- `src/chatView.ts`
  - Webview chat UI provider (VTCode Chat panel).
...
3. In the Extension Development Host:
   ...
   - Ensure `vtcode` is on PATH (see status bar or VTCode views).

Verification checklist:

- `VTCode` status bar item appears.
- `VTCode Quick Actions` and `VTCode Workspace Status` views show in the Explorer / View container.
- `VTCode: Ask the Agent` command is available.
- `VTCode: Open Chat` opens the VTCode chat view.
...
Use this Quick Start as your checklist to get productive on the VTCode VSCode extension within minutes.