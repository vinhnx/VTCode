...
const TOOLS_POLICY_LABEL: &str = "Tools policy – prompts before running elevated actions (recommended)";
const FULL_AUTO_LABEL: &str = "Full auto – allow unattended execution without prompts";

pub(crate) fn prompt_trust(renderer: &mut AnsiRenderer, default: WorkspaceTrustLevel) -> Result<WorkspaceTrustLevel> {
...
fn prompt_trust_text(renderer: &mut AnsiRenderer, default: WorkspaceTrustLevel) -> Result<WorkspaceTrustLevel> {
    ...
    loop {
        ...
        match trimmed {
            ...
            _ => {
                renderer.line(MessageStyle::Error, "Please choose 1 for Tools policy or 2 for Full auto.")?;
            }
        }
    }
}
...
fn trust_entries() -> [(WorkspaceTrustLevel, SelectionEntry); 2] {
    [
        (WorkspaceTrustLevel::ToolsPolicy, SelectionEntry::new(TOOLS_POLICY_LABEL.to_owned(), None)),
...
fn select_trust_with_ratatui(default: WorkspaceTrustLevel) -> Result<WorkspaceTrustLevel> {
    ...
    let selection_entries: Vec<SelectionEntry> = entries.iter().map(|(_level, entry)| entry.clone()).collect();
    let instructions = format!(
        "Default: {}. Use ↑/↓ or j/k to choose, Enter to confirm, Esc to keep the default.",
        selection_entries[default_index].title
    );
    let selected_index = run_selection("Workspace trust", &instructions, &selection_entries, default_index)?;
