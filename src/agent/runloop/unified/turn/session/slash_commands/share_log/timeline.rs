...
fn timeline_row_from_item(
    ...
    match &item.details {
        ...
        ThreadItemDetails::ToolInvocation(tool) => {
            ...
            let tool_name = super::canonical_tool_name(&tool.tool_name);
            let mut body = String::new();
...
pub(super) fn redact_timeline_export(export: &TimelineExport) -> TimelineExport {
    ...
    redacted.workspace = super::redact_sensitive_text(&redacted.workspace);
    redacted.thread_id = super::redact_sensitive_text(&redacted.thread_id);
    if let Some(outcome_code) = &redacted.overview.outcome_code {
        redacted.overview.outcome_code = Some(super::redact_sensitive_text(outcome_code));
    }
    for row in &mut redacted.rows {
        row.title = super::redact_sensitive_text(&row.title);
        row.summary = super::redact_sensitive_text(&row.summary);
        row.body = super::redact_sensitive_text(&row.body);
        row.turn_id = row
            .turn_id
            .as_ref()
            .map(|value| super::redact_sensitive_text(value));
