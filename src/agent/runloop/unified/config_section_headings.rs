...
static SECTION_HEADINGS: &[(&str, &str, &str)] = &[
    ...
    ("acp.zed", "Zed Integration", "How VT Code connects commands and trust settings to Zed."),
    ("acp.zed.auth", "Zed Authentication", "Default authentication flow for ACP sessions in Zed."),
    ...
    ("agent.harness", "Harness Limits", "Per-turn tool call and timing safety limits."),
    ("agent.onboarding", "Onboarding", "Welcome message, tips, and first-run guidance."),
    ("agent.open_responses", "Open Responses", "Compatibility events and response item mapping."),
    (
    ...
    ("agent.vibe_coding", "Vibe Coding", "Loose-input parsing and extra contextual assistance."),
    ("auth", "Authentication", "Provider login and token handling."),
    ("auth.openrouter", "OpenRouter OAuth", "Browser login flow and token refresh behavior."),
    ("automation", "Automation", "Autonomous run controls and safety defaults."),
    ("automation.full_auto", "Full Auto", "Guardrails for unattended execution."),
    (
    ...
    ("mcp.security", "MCP Security", "Validation and runtime protection for MCP."),
    (
        "mcp.security.rate_limit",
        "MCP Rate Limits",
        "Concurrency and throughput guardrails for MCP traffic.",
    ),
    ("mcp.security.validation", "MCP Validation", "Schema and payload checks for MCP requests."),
    ("mcp.server", "MCP Server Transport", "How VT Code hosts its own MCP server."),
    ...
    ("provider.openai", "OpenAI", "OpenAI-specific request settings."),
    (
        "provider.openai.tool_search",
        "OpenAI Tool Search",
        "Hosted search tool configuration for OpenAI Responses models.",
    ),
    ("pty", "Terminal Sessions", "PTY limits, buffering, and shell defaults."),
    ("sandbox", "Sandboxing", "Process isolation and filesystem boundaries."),
    ("sandbox.external", "External Sandbox", "Container or VM-backed isolation settings."),
    ...
    ("syntax_highlighting", "Syntax Highlighting", "Language-aware rendering in the transcript."),
    ("telemetry", "Telemetry", "Local analytics, metrics, and reporting."),
