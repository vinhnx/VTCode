...
pub(crate) fn wait_timeout_warning_message(subject: &str, timeout: Duration, remaining: Duration) -> String {
    format!(
...
mod tests {
    ...
    fn warning_delay_prefers_earlier_desired_delay() {
        assert_eq!(
            resolve_warning_delay(Duration::from_secs(60), Duration::from_secs(12), Duration::from_secs(5)),
    ...
    fn warning_delay_caps_at_timeout_headroom() {
        assert_eq!(
            resolve_warning_delay(Duration::from_secs(60), Duration::from_secs(58), Duration::from_secs(10)),
    ...
    fn warning_delay_skips_zero_delay() {
        assert_eq!(resolve_warning_delay(Duration::from_secs(5), Duration::from_secs(5), Duration::from_secs(5)), None);
    }

    #[test]
    fn fractional_warning_delay_targets_three_quarters_budget() {
        assert_eq!(
            resolve_fractional_warning_delay(Duration::from_secs(60), 0.75, Duration::from_secs(15)),
