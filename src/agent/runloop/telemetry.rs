...
pub(crate) fn build_trajectory_logger(workspace: &Path, vt_cfg: Option<&VTCodeConfig>) -> TrajectoryLogger {
    match vt_cfg {
        ...
        Some(cfg) => {
            let retention = TrajectoryRetention {
                ...
                max_total_size_bytes: cfg.telemetry.trajectory_max_size_mb.saturating_mul(BYTES_PER_MB),
            };
