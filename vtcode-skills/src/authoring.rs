...
pub type SkillFrontmatter = crate::manifest::SkillYaml;
            ...
            for key in map.keys() {
                if !crate::manifest::SUPPORTED_FRONTMATTER_KEYS.contains(&key.as_str()) {
                    report.errors.push(format!(
                        ...
                        key,
                        crate::manifest::SUPPORTED_FRONTMATTER_KEYS.join(", ")
                    ));
                }
            }
        }
...
pub fn render_skills_lean(skills: &[crate::types::Skill]) -> Option<String> {
    if skills.is_empty() {
        return None;
    }
    ...
    for skill in sorted_skills {
        ...
        let scope = match skill.scope {
            ...
            crate::types::SkillScope::Repo => "repo",
...
"#,
    ...
    async fn test_render_skills_lean() {
        ...
        // Load skills
        use crate::manifest::parse_skill_file;
