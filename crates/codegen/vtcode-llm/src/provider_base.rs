...
use crate::provider::{LLMError, LLMRequest, LLMStreamEvent};
...
trait OpenAICompatibleProvider: Send + Sync {
    ...
    fn parse_openai_request(&self, value: &Value, default_model: &str) -> Option<LLMRequest> {
        crate::utils::parse_chat_request_openai_format(value, default_model)
    }

    /// Serialize messages to OpenAI format
    fn serialize_openai_messages(&self, request: &LLMRequest) -> Value {
        use crate::providers::common::serialize_messages_openai_format;
...
impl StreamProcessor {
    ...
    fn parse_stream_event(&self, json: Value) -> Option<LLMStreamEvent> {
        ...
        crate::utils::parse_stream_event_openai_format(json, self.provider_name)
    }
