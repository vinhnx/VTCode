...
pub struct Response {
    ...
    error: Option<Box<OpenResponseError>>,
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub(crate) usage: Option<Box<OpenUsage>>,
...
impl Response {
    ...
    pub(crate) fn add_output(&mut self, item: OutputItem) {
    ...
    pub fn complete(&mut self) {
        ...
        self.completed_at = Some(SystemTime::now().duration_since(UNIX_EPOCH).map(|d| d.as_secs()).unwrap_or(0));
    }

    /// Marks the response as failed with the given error.
    pub(crate) fn fail(&mut self, error: OpenResponseError) {
...
mod tests {
    ...
    fn boxed_sparse_fields_are_smaller_than_inline_options() {
        ...
        assert!(size_of::<Option<Box<OpenResponseError>>>() < size_of::<Option<OpenResponseError>>());
    }
}
