...
impl LifecycleHookEngine {
    async fn base_payload(&self, hook_event_name: &str) -> Result<serde_json::Map<String, Value>> {
        ...
        Ok(serde_json::Map::from_iter([
            ...
            ("transcript_path".to_string(), transcript_path.map(Value::String).unwrap_or(Value::Null)),
        ]))
    }
    ...
    pub(crate) async fn build_session_end_payload(&self, turn_id: &str, reason: SessionEndReason) -> Result<Value> {
    ...
    pub(crate) async fn build_subagent_start_payload(
        ...
        payload.insert("parent_session_id".to_string(), Value::String(parent_session_id.to_owned()));
    ...
    pub(crate) async fn build_user_prompt_payload(&self, turn_id: &str, prompt: &str) -> Result<Value> {
    ...
    pub(super) async fn build_permission_request_payload(
        ...
        payload.insert("tool_name".to_string(), Value::String(tool_name.to_owned()));
        payload.insert("tool_input".to_string(), tool_input.cloned().unwrap_or(Value::Null));
        payload.insert("permission_request".to_string(), build_permission_request_summary(permission_request));
        payload.insert("permission_suggestions".to_string(), Value::Array(permission_suggestions.to_vec()));
        Ok(Value::Object(payload))
    }
    ...
    pub(super) async fn build_stop_payload(
        ...
        payload.insert("last_assistant_message".to_string(), Value::String(last_assistant_message.to_owned()));
    ...
    pub(super) async fn build_notification_payload(
        ...
        payload.insert("notification_type".to_string(), Value::String(notification_type.as_str().to_owned()));
...
fn build_permission_request_summary(permission_request: &PermissionRequest) -> Value {
    let (kind, details) = match &permission_request.kind {
        ...
        PermissionRequestKind::Mcp { server, tool } => ("mcp", json!({ "server": server, "tool": tool })),
