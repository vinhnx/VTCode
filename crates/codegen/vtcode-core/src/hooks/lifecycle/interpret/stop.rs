...
pub(crate) fn interpret_stop(
    ...
    if let Some(stderr) = trimmed_non_empty(&result.stderr) {
        outcome
            .messages
            .push(HookMessage::warning(format!("Stop hook `{}` stderr: {}", command.command, stderr)));
    }
    ...
    let Some(json) = parse_json_output(&result.stdout) else {
        if !result.stdout.trim().is_empty() {
            if looks_like_json(&result.stdout) {
                outcome
                    .messages
                    .push(HookMessage::error(format!("Stop hook `{}` returned invalid JSON output", command.command)));
        ...
        && matches_hook_event(&spec, "Stop")
    {
        if let Some(decision) = spec.get("decision").and_then(Value::as_str)
            && decision.eq_ignore_ascii_case("block")
        {
            outcome.block_reason = spec.get("reason").and_then(Value::as_str).and_then(trimmed_non_empty);
        } else if spec.get("continue").and_then(Value::as_bool) == Some(false) {
            outcome.block_reason = spec.get("stopReason").and_then(Value::as_str).and_then(trimmed_non_empty);
        }
    }
}
