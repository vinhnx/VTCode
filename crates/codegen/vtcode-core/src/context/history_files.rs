...
//! is written to `.vtcode/history/` so agents can recover details with shell search.
//!
//! ## Design Philosophy
//!
...
//! 3. Agent can search history with `rg` when details are needed
...
mod tests {
    ...
    fn test_format_summary_with_reference() {
        ...
        assert!(summary.contains("exec_command.cmd"));
    }
