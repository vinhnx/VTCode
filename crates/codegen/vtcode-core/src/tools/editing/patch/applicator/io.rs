...
impl AtomicWriter {
    pub(crate) async fn create(path: &Path, permissions: Option<Permissions>) -> Result<Self, PatchError> {
        ...
        let file = fs::File::create(&temp_path).await.map_err(|err| PatchError::Io {
            action: "create",
    ...
    pub(crate) async fn write_all(&mut self, bytes: &[u8]) -> Result<(), PatchError> {
        self.writer.write_all(bytes).await.map_err(|err| PatchError::Io {
            action: "write",
...
impl BackupEntry {
    async fn create(target: &Path, kind: BackupKind) -> Result<Self, PatchError> {
        ...
        fs::rename(target, &backup_path).await.map_err(|err| PatchError::Io {
            action: "rename",
            path: target.to_path_buf(),
            source: err,
        })?;
        Ok(Self { path: backup_path, kind })
    }
    ...
    pub(super) async fn remove(self) -> Result<(), PatchError> {
        match self.kind {
            BackupKind::File => match fs::remove_file(&self.path).await {
                Ok(()) => Ok(()),
                Err(err) if err.kind() == ErrorKind::NotFound => Ok(()),
                Err(err) => Err(PatchError::Io { action: "delete", path: self.path, source: err }),
            },
    ...
    pub(super) async fn restore(&self, destination: &Path) -> Result<(), PatchError> {
        match fs::rename(&self.path, destination).await {
            ...
            Err(err) if err.kind() == ErrorKind::AlreadyExists => {
                ...
                fs::rename(&self.path, destination).await.map_err(|rename_err| PatchError::Io {
                    action: "rename",
...
pub(super) async fn remove_existing_entry(path: &Path) -> Result<(), PatchError> {
    match fs::metadata(path).await {
        Ok(metadata) => {
            if metadata.is_dir() {
                fs::remove_dir_all(path).await.map_err(|err| PatchError::Io {
                    action: "delete",
                    path: path.to_path_buf(),
                    source: err,
                })?
...
fn temporary_path(target: &Path) -> Result<PathBuf, PatchError> {
    ...
    let file_name = target.file_name().and_then(|name| name.to_str()).unwrap_or("vtcode-patch");
    let timestamp = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .map_err(|err| PatchError::TempPath { path: target.to_path_buf(), source: err })?
        .as_nanos();
