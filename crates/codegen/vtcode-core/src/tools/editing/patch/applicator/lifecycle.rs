...
pub(crate) enum OperationEffect {
    Applied { state: OperationState, detail: String },
...
impl OperationState {
    pub(crate) async fn commit(self) -> Result<(), PatchError> {
        match self {
            ...
            OperationState::DeletedFile { backup, .. } | OperationState::UpdatedFile { backup, .. } => {
                backup.remove().await
            }
        }
    }
    ...
    pub(crate) async fn rollback(self) -> Result<(), PatchError> {
        match self {
            ...
            OperationState::DeletedFile { original_path, backup } => backup.restore(&original_path).await,
