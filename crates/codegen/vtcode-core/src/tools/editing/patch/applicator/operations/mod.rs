...
impl<'a> Operation<'a> {
    pub(crate) fn from_prepared(prepared: PreparedOperation<'a>) -> Self {
        match prepared {
            PreparedOperation::Add { path, content } => Operation::Add(add::AddOperation::new(path, content)),
            PreparedOperation::Delete { path } => Operation::Delete(delete::DeleteOperation::new(path)),
