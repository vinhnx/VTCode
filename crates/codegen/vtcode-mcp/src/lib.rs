...
//! reporting) is original; only the protocol interaction layer mirrors
//! Codex' `mcp-client` crate.
//!
...
pub fn validate_mcp_config(config: &vtcode_config::mcp::McpClientConfig) -> Result<()> {
...
mod tests {
    ...
    fn validate_elicitation_payload_rejects_invalid_content() {
        use crate::rmcp_client::{build_elicitation_validator, validate_elicitation_payload};
