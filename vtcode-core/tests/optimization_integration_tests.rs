...
use vtcode_core::tools::async_pipeline::{
    AsyncToolPipeline, ToolRequest, ExecutionPriority, ExecutionContext
};
...
async fn test_optimized_tool_registry() -> Result<()> {
    ...
    // Test tool execution
    let result = registry.execute_tool_optimized(
        "test_tool",
...
async fn test_optimized_agent_engine() -> Result<()> {
    ...
    let llm_client = Arc::new(OptimizedLLMClient::new(
        4,
        50,
        10.0,
        20,
    ...
    let _engine = OptimizedAgentEngine::new(
...
async fn test_performance_under_load() -> Result<()> {
    ...
    for i in 0..100 {
        let request = ToolRequest {
            ...
            priority: if i % 10 == 0 { ExecutionPriority::High } else { ExecutionPriority::Normal },
