...
impl SkillLocationType {
    ...
    fn from_path(path: &Path) -> Option<Self> {
        ...
        if path_str.contains(".vtcode/skills") {
            if path_str.contains("~/") || path_str.contains("/home/") || path_str.contains("/Users/") {
...
impl SkillLocations {
    ...
    pub fn discover_skills(&self) -> Result<Vec<DiscoveredSkill>> {
        ...
        info!("Discovery complete: {} skills found ({} from higher precedence locations)", 
        ...
        final_skills.sort_by(|a, b| {
            match a.location_type.cmp(&b.location_type) {
                ...
                Ok((manifest, _)) => {
                    ...
                    if let Some(existing) = discovered.get(&manifest.name) {
                        if existing.location_type < location.location_type {
                            ...
                            debug!("Skipping skill '{}' from {} (already exists from higher precedence {})", 
                    ...
                    info!("Discovered skill: '{}' from {} at {}", 
                        manifest.name, location.location_type, dir.display());
                ...
                if let Some(skill_name) = location.get_skill_name(&path) {
                    match parse_skill_file(&path) {
                        Ok((manifest, _)) => {
                            ...
                            stats.skills_found += 1;
                            info!("Discovered skill: '{}' from {} at {}", 
                                manifest.name, location.location_type, path.display());
                        }
                        Err(e) => {
    ...
    pub fn get_location_types(&self) -> Vec<SkillLocationType> {
        self.locations.iter().map(|loc| loc.location_type.clone()).collect()
    }
...
mod tests {
    ...
    async fn test_location_discovery() {
        ...
        let skill_names: Vec<String> = discovered.iter()
    ...
    fn test_full_integration() {
        ...
        let vtcode_skills = workspace_root.join(".vtcode/skills");
        let vtcode_locations = SkillLocations::with_locations(vec![
            SkillLocation::new(
                SkillLocationType::VtcodeProject,
                vtcode_skills,
                true,
        ...
        println!("VTCode location discovered {} skills", vtcode_discovered.len());
        
        // Verify we found the expected skills
        let skill_names: Vec<String> = vtcode_discovered.iter()
            .map(|d| d.skill_context.manifest().name.clone())
            .collect();
