...
impl SkillAuthor {
	...
	pub fn validate_skill(&self, skill_dir: &Path) -> Result<ValidationReport> {
		...
		let raw_frontmatter: serde_yaml::Value = serde_yaml::from_str(parts[1].trim())
			...
			for key in map.keys() {
				if let serde_yaml::Value::String(key_str) = key {
					if !allowed.contains(&key_str.as_str()) {
						report.errors.push(format!("Unexpected property '{}' in frontmatter. Allowed: {}", key_str, allowed.join(", ")));
					}
				}
			}
		}
		...
		if frontmatter.description.is_empty() {
			...
			if frontmatter.description.contains('<') || frontmatter.description.contains('>') {
				report.errors.push("Description cannot contain angle brackets (< or >)".to_string());
			}
			if frontmatter.description.len() > 1024 {
				report.errors.push(format!("Description is too long ({} characters). Maximum is 1024 characters.", frontmatter.description.len()));
			}
		}
		...
		if body.len() > 50000 {
			report.warnings.push("SKILL.md body is very long (>50k chars). Consider splitting into reference files.".to_string());
		}
	...
	fn validate_structure(&self, skill_dir: &Path, report: &mut ValidationReport) -> Result<()> {
		...
		if skill_dir.join("README.md").exists() {
			report.warnings.push(
				"README.md found - not needed for skills (use SKILL.md only)".to_string(),
			);
		}
		...
		if skill_md_content.contains('\\') {
			report.warnings.push(
				"SKILL.md contains backslashes - use forward slashes for paths".to_string(),
			);
		}
...
pub fn render_skills_lean(skills: &[crate::skills::types::Skill]) -> Option<String> {
	...
	let user_skills: Vec<_> = skills.iter().filter(|s| matches!(s.scope, crate::skills::types::SkillScope::User)).collect();
	let repo_skills: Vec<_> = skills.iter().filter(|s| matches!(s.scope, crate::skills::types::SkillScope::Repo)).collect();
...
mod tests {
	...
	async fn test_create_skill() {
		...
		let skill_dir = author.create_skill("test-skill", Some(tmp.path().to_path_buf())).unwrap();
	...
	async fn test_render_skills_lean() {
		...
		let skill2_dir = author.create_skill("spreadsheet-generator", Some(tmp.path().to_path_buf())).unwrap();

		// Update descriptions to be valid
		let skill1_md = skill1_dir.join("SKILL.md");
		let content1 = fs::read_to_string(&skill1_md).unwrap()
			.replace("[TODO: Complete and informative explanation", "Extract text and tables from PDFs");
		fs::write(skill1_md, content1).unwrap();

		let skill2_md = skill2_dir.join("SKILL.md");
		let content2 = fs::read_to_string(&skill2_md).unwrap()
			.replace("[TODO: Complete and informative explanation", "Create Excel spreadsheets with charts");
