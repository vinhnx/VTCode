...
use crate::marketplace::{
    ...
    MarketplaceSource,
    PluginManifest,
    config::{MarketplaceSettings, InstalledPlugin}
};
...
pub fn test_plugin_validation() -> Result<()> {
    ...
    let temp_dir = tempfile::tempdir()?;
    let installer = super::installer::PluginInstaller::new(
    ...
    assert!(invalid_result.is_err(), "Invalid plugin should fail validation");
