...
use crate::llm::provider::{LLMProvider, LLMRequest, Message, MessageRole, ToolDefinition, LLMStreamEvent};
use axum::{
    extract::State,
    http::StatusCode,
...
pub async fn messages_handler(
    ...
    if request.stream {
        ...
        let stream = match state.provider.stream(llm_request).await {
            Ok(s) => s,
            Err(_) => return Ok((StatusCode::INTERNAL_SERVER_ERROR, "Stream error").into_response()),
        };
        ...
        tokio::spawn(async move {
            ...
            if tx.send(Event::default().json_data(AnthropicStreamEvent::MessageStart { message: initial_response })).await.is_err() {
                return;
            }
            ...
            while let Some(event_result) = stream.next().await {
                match event_result {
                    Ok(provider_event) => {
                        match provider_event {
                            LLMStreamEvent::Token { delta } => {
                                ...
                                if tx.send(Event::default().json_data(AnthropicStreamEvent::ContentBlockStart {
                                ...
                                // Send content delta
                                if tx.send(Event::default().json_data(AnthropicStreamEvent::ContentBlockDelta {
                                    index: content_block_idx,
                                ...
                                if tx.send(Event::default().json_data(AnthropicStreamEvent::ContentBlockStop {
                                    index: content_block_idx,
                                })).await.is_err() {
                                    break;
                                }
                            ...
                            LLMStreamEvent::Completed { response } => {
                                ...
                                // Send message delta
                                if tx.send(Event::default().json_data(AnthropicStreamEvent::MessageDelta {
                                    delta,
                                    usage: AnthropicUsage {
                                ...
                                // Send message stop
                                if tx.send(Event::default().json_data(AnthropicStreamEvent::MessageStop {})).await.is_err() {
                                    break;
                                }
                    ...
                    Err(e) => {
                        ...
                        if tx.send(Event::default().json_data(error_event)).await.is_err() {
                            break;
                        }
                        break;
                    }
        ...
        let response = match state.provider.generate(llm_request).await {
            Ok(r) => r,
            Err(_) => return Ok((StatusCode::INTERNAL_SERVER_ERROR, "Generation error").into_response()),
        };
        
        let anthropic_response = match convert_llm_to_anthropic_response(response) {
            Ok(resp) => resp,
            Err(_) => return Ok((StatusCode::INTERNAL_SERVER_ERROR, "Conversion error").into_response()),
        };
...
/// Convert Anthropic request to VT Code LLM request
fn convert_anthropic_to_llm_request(
                ...
                blocks.iter()
                    .filter_map(|block| {
                        match block {
                            ...
                            AnthropicContentBlock::ToolResult { content: tool_content, tool_use_id, .. } => {
                                let content_text = match tool_content {
                                    AnthropicContent::Text(text) => text.clone(),
                                    AnthropicContent::Blocks(inner_blocks) => {
                                        inner_blocks.iter()
