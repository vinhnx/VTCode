...
impl ModelsDatabase {
    ...
    pub fn from_file(path: &Path) -> Result<Self> {
        ...
        let json: JsonValue = serde_json::from_str(&content)
            .context("Failed to parse models database JSON")?;
...
impl ConfigValidator {
    ...
    pub fn validate(&self, config: &VTCodeConfig) -> Result<ValidationResult> {
        ...
        if !self.models_db.model_exists(&config.agent.provider, &config.agent.default_model) {
        ...
        if let Some(max_tokens) = self.models_db.get_context_window(
            &config.agent.provider,
        ...
        if let Ok(cwd) = std::env::current_dir() {
            ...
            if !cwd.exists() {
                result.warnings.push("Current working directory does not exist".to_string());
            }
        }
    ...
    pub fn quick_validate(&self, config: &VTCodeConfig) -> Result<()> {
        ...
        // Check API key
        get_api_key(&config.agent.provider, &ApiKeySources::default())
            .with_context(|| {
                format!(
                    "API key not found for provider '{}'. Set {} environment variable.",
                    config.agent.provider,
...
mod tests {
  ...
  "openai": {
    ...
    fn gets_context_window() {
        ...
        assert_eq!(db.get_context_window("google", "gemini-2.5-flash"), Some(1000000));
