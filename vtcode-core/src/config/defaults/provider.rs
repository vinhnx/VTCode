...
static CONFIG_DEFAULTS: Lazy<RwLock<Arc<dyn ConfigDefaultsProvider>>> =
    Lazy::new(|| RwLock::new(Arc::new(DefaultConfigDefaults::default())));
...
pub fn reset_to_default_config_defaults() {
    let _ = install_config_defaults_provider(Arc::new(DefaultConfigDefaults::default()));
}
    ...
    P: WorkspacePaths + 'static,
{
    ...
    /// Overrides the configuration file name searched within the workspace.
    pub fn with_config_file_name(mut self, name: impl Into<String>) -> Self {
        self.config_file_name = name.into();
        self
    }

    /// Overrides the fallback configuration search paths.
    pub fn with_home_paths<I, Pth>(mut self, home_paths: I) -> Self
    where
        I: IntoIterator<Item = Pth>,
        Pth: Into<PathBuf>,
    {
        self.home_paths = Some(home_paths.into_iter().map(Into::into).collect());
        self
    }

    /// Overrides the default syntax highlighting theme identifier.
    pub fn with_syntax_theme(mut self, theme: impl Into<String>) -> Self {
        self.syntax_theme = theme.into();
        self
    }

    /// Overrides the default syntax highlighting language list.
    pub fn with_syntax_languages<I, S>(mut self, languages: I) -> Self
    where
        I: IntoIterator<Item = S>,
        S: Into<String>,
    {
        self.syntax_languages = languages.into_iter().map(Into::into).collect();
        self
    }
}
    ...
    P: WorkspacePaths + 'static,
{
    ...
    fn workspace_paths_for(&self, _workspace_root: &Path) -> Box<dyn WorkspacePaths> {
        Box::new(ArcWorkspacePaths::new(self.paths.clone()))
    }
...
impl<P> WorkspacePaths for ArcWorkspacePaths<P>
where
    P: WorkspacePaths + ?Sized,
{
    fn workspace_root(&self) -> &Path {
