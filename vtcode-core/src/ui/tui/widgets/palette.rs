        ...
        if list_height > 0 {
            ...
            if self.palette.has_more_items() {
                ...
                list_items.push(ListItem::new(Line::from(Span::styled(
                    format!(
                        "  ... ({} more items)",
                        self.palette.total_items() - (self.palette.current_page_number() * 20)
                    ),
                    continuation_style,
                ))));
            }
...
/// Widget for rendering the custom prompts palette
///
/// # Example
/// ```ignore
/// PromptPaletteWidget::new(session, palette, viewport)
///     .highlight_style(accent_style)
///     .render(area, buf);
/// ```
pub struct PromptPaletteWidget<'a> {
    session: &'a Session,
    palette: &'a PromptPalette,
    viewport: Rect,
    highlight_style: Option<Style>,
}

impl<'a> PromptPaletteWidget<'a> {
    /// Create a new PromptPaletteWidget with required parameters
    pub fn new(session: &'a Session, palette: &'a PromptPalette, viewport: Rect) -> Self {
        Self {
            session,
            palette,
            viewport,
...
impl<'a> Widget for PromptPaletteWidget<'a> {
    fn render(self, _area: Rect, buf: &mut Buffer) {
        if self.viewport.height == 0 || self.viewport.width == 0 {
            return;
        }
