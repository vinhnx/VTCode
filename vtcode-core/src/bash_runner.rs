...
impl BashRunner {
    ...
    pub fn ls(&self, path: Option<&str>, show_hidden: bool) -> Result<String> {
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute ls command".to_owned())?;
    ...
    pub fn mkdir(&self, path: &str, parents: bool) -> Result<()> {
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute mkdir command".to_owned())?;
    ...
    pub fn rm(&self, path: &str, recursive: bool, force: bool) -> Result<()> {
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute rm command".to_owned())?;
    ...
    pub fn cp(&self, source: &str, dest: &str, recursive: bool) -> Result<()> {
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute cp command".to_owned())?;
    ...
    pub fn mv(&self, source: &str, dest: &str) -> Result<()> {
        ...
        let output = Command::new("mv")
            ...
            .arg(&dest_path)
            .output()
            .with_context(|| "Failed to execute mv command".to_owned())?;
    ...
    pub fn grep(&self, pattern: &str, path: Option<&str>, recursive: bool) -> Result<String> {
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute grep command".to_owned())?;
    ...
    pub fn find(
        ...
        let output = cmd
            .output()
            .with_context(|| "Failed to execute find command".to_owned())?;
    ...
    pub fn cat(
        ...
        if let (Some(start), Some(end)) = (start_line, end_line) {
            ...
            let output = Command::new("sed")
                ...
                .arg(&file_path)
                .output()
                .with_context(|| "Failed to execute sed command".to_owned())?;
            ...
            let output = Command::new("cat")
                .arg(&file_path)
                .output()
                .with_context(|| "Failed to execute cat command".to_owned())?;
    ...
    pub fn head(&self, path: &str, lines: usize) -> Result<String> {
        ...
        let output = Command::new("head")
            ...
            .arg(&file_path)
            .output()
            .with_context(|| "Failed to execute head command".to_owned())?;
    ...
    pub fn tail(&self, path: &str, lines: usize) -> Result<String> {
        ...
        let output = Command::new("tail")
            ...
            .arg(&file_path)
            .output()
            .with_context(|| "Failed to execute tail command".to_owned())?;
