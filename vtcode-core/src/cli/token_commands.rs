...
pub async fn handle_token_command(command: TokenCommands) -> Result<(), Box<dyn std::error::Error>> {
...
async fn handle_status_command(token_budget: Arc<TokenBudgetManager>) -> Result<(), Box<dyn std::error::Error>> {
    let report = token_budget.generate_report().await;
    println!("{}", report);
    Ok(())
}

async fn handle_history_command(token_budget: Arc<TokenBudgetManager>) -> Result<(), Box<dyn std::error::Error>> {
...
async fn handle_summary_command(token_budget: Arc<TokenBudgetManager>) -> Result<(), Box<dyn std::error::Error>> {
