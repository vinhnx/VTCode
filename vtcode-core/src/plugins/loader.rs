...
use super::{PluginResult, PluginError, PluginRuntime, PluginManifest};
...
impl PluginLoader {
    ...
    pub async fn install_plugin(&self, source: PluginSource, name: Option<String>) -> PluginResult<()> {
        let plugin_dir = match source {
            PluginSource::Local(path) => {
                self.install_from_local(path).await?
            }
            PluginSource::Git(url) => {
                self.install_from_git(&url, name.as_deref()).await?
            }
            PluginSource::Http(url) => {
                self.install_from_http(&url, name.as_deref()).await?
            }
    ...
    async fn install_from_local(&self, source_path: PathBuf) -> PluginResult<PathBuf> {
        ...
        if !manifest_path.exists() {
            return Err(PluginError::ManifestValidationError(
                format!("Plugin manifest not found in source: {}", manifest_path.display())
            ));
        }
    ...
    async fn install_from_git(&self, url: &str, name: Option<&str>) -> PluginResult<PathBuf> {
        ...
        let temp_dir = TempDir::new()
            .map_err(|e| PluginError::LoadingError(format!("Failed to create temporary directory: {}", e)))?;
        ...
        let plugin_name = name.map(|s| s.to_string())
  ...
  "description": "Placeholder for HTTP-installed plugin from {}"
        ...
        fs::write(&manifest_path, placeholder_manifest).await
    ...
    async fn install_from_marketplace(&self, marketplace_id: &str, name: Option<&str>) -> PluginResult<PathBuf> {
        // For now, create a placeholder implementation
        let plugin_name = name.map(|s| s.to_string())
            .unwrap_or_else(|| marketplace_id.split('/').last().unwrap_or(marketplace_id).to_string());
  ...
  "description": "Placeholder for marketplace-installed plugin from {}"
    ...
    pub async fn uninstall_plugin(&self, plugin_name: &str) -> PluginResult<()> {
        ...
        fs::remove_dir_all(&plugin_dir).await
            .map_err(|e| PluginError::LoadingError(format!("Failed to remove plugin directory: {}", e)))?;
    ...
    pub async fn list_installed_plugins(&self) -> PluginResult<Vec<String>> {
        ...
        let mut entries = fs::read_dir(&self.plugins_dir).await
            .map_err(|e| PluginError::LoadingError(format!("Failed to read plugins directory: {}", e)))?;
