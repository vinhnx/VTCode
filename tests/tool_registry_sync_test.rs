...
fn test_tool_policy_categories_documented() {
    ...
    assert_eq!(tool_policies.len(), 26, "Policy count mismatch - verify all tools documented");
}
...
fn test_acp_tool_subset_is_documented() {
    ...
    let acp_tools: HashSet<&str> = HashSet::from_iter(vec!["read_file", "list_files"].iter().map(|s| *s));
    
    let all_tools: HashSet<_> = HashSet::from_iter(vec![
        "list_files",
        "grep_file",
        "read_file",
        "write_file",
    ...
    let tools_excluded_from_acp: Vec<_> = all_tools
        .difference(&acp_tools)
        .copied()
        .collect();
    ...
    assert_eq!(acp_tools.len(), 2, "ACP tool count changed - verify this is intentional");
    assert!(acp_tools.contains("read_file"), "read_file should be available via ACP");
    assert!(acp_tools.contains("list_files"), "list_files should be available via ACP");
}
...
fn test_tool_policy_consistency() {
    ...
    assert_eq!(expected_policies.len(), 26, "Policy mapping completeness check");
}
