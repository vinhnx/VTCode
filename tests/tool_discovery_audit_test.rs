...
fn audit_tool_system_completeness() {
    ...
    println!("  With policies: {}/{}", tools_with_policies.len(), audit_entries.len());
    println!("  With declarations: {}/{}", tools_with_declarations.len(), audit_entries.len());
    ...
    if !missing_declarations.is_empty() {
        println!("\n  ℹ️  TOOLS WITHOUT DECLARATIONS: {:?}", missing_declarations);
...
fn audit_tool_categories() {
    ...
    println!("Total: {} tools across {} categories\n", total_tools, categories.len());
}
...
fn audit_acp_tool_selection_reasoning() {
    ...
    let excluded = vec![
        ("grep_file", "Functionality overlaps with Zed's native search"),
        ...
        ("run_pty_cmd", "Terminal access not available in Zed context"),
        ...
        ("*_pty_session", "Terminal operations not available in editor"),
    ];
