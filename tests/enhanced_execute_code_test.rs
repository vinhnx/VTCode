...
"#;
    ...
    assert!(result.get("stdout").unwrap().as_str().unwrap().contains("Hello World"));
...
"#;
    ...
    println!("✓ Multiple file detection test passed: found {} files", files_array.len());
}
...
"#;
    ...
    println!("Execution result: {}", serde_json::to_string_pretty(&exec_result).unwrap());
...
"#,
    ...
    let exec_result_new = registry.execute("execute_code", exec_args_new).await.unwrap();
    println!("Execution with tracking: {}\n", serde_json::to_string_pretty(&exec_result_new).unwrap());
