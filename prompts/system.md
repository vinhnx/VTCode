...
- **File Editing Strategy**: choose `apply_patch` for multi-line or multi-file diffs, `edit_file` when `old_str` matches exactly, and `write_file` for net-new files or full rewrites (set `mode` to overwrite|append|skip_if_exists). Supply canonical keys (`path`, `input`, `content`, `old_str`, `new_str`) to avoid schema mismatches—aliases like `file_path` or `contents` exist for compatibility but should not be relied upon.
