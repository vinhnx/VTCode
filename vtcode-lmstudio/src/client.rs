//! Low-level LM Studio API client implementation.
//!
//! Handles communication with LM Studio servers via HTTP endpoints for model
//! discovery, loading, and inference.

use std::io;
use std::path::Path;
use vtcode_config::VTCodeConfig;

/// LM Studio HTTP client for model management and inference.
#[derive(Clone)]
...
impl LMStudioClient {
    ...
    ///
    /// This verifies that the server is reachable before returning.
    pub async fn try_from_provider(_config: &VTCodeConfig) -> io::Result<Self> {
        // Get the LM Studio base URL from environment or use default
        let base_url = std::env::var("LMSTUDIO_URL")
            .ok()
            .or_else(|| Some("http://localhost:1234".to_string()))
            .ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    "LM Studio base_url not configured. Set LMSTUDIO_URL environment variable.",
                )
            })?;
    ...
    /// Check if the LM Studio server is accessible.
    ...
    /// Load a model by sending a minimal request to warm up the server.
    ///
    /// This sends an empty input with max_output_tokens=1 to trigger model loading.
    ...
    /// Fetch the list of models available on the LM Studio server.
    ...
    /// Find the `lms` CLI tool in PATH or fallback locations.
    fn find_lms() -> io::Result<String> {
        Self::find_lms_with_home_dir(None)
    }

    /// Find the `lms` CLI tool with optional home directory override.
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var("VTCODE_SANDBOX_NETWORK_DISABLED").is_ok() {
            tracing::info!("VTCODE_SANDBOX_NETWORK_DISABLED is set; skipping test");
            return;
        }
