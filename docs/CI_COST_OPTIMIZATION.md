...
This document outlines the cost optimization strategies implemented for VTCode's release workflow.
...
**Note:** VTCode is a public repository, so all GitHub Actions usage is **FREE** (unlimited minutes). However, these optimizations still matter for:
