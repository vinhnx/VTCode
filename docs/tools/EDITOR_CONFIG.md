...
When no `preferred_editor` is specified, VTCode uses a two-stage detection process:
...
If neither environment variable is set, VTCode tries common editors in PATH:
...
This prevents terminal corruption and input conflicts when switching between VTCode and external editors.

## Troubleshooting

### "Failed to detect editor" error

This error means no editor was found. VTCode checks both environment variables and fallback editors.
...
## Integration with VTCode Features
...
- [VTCode configuration guide](../config/CONFIGURATION_PRECEDENCE.md)
