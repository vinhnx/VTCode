...
For delegated child-agent workflows and top-level agent overlays, see the dedicated [Subagents guide](./subagents.md). New `.vtcode/agents/*.md` files should use VT Code tool ids such as `read_file`, `list_files`, `unified_search`, and `unified_exec`, not Claude-style names like `Read` or `Bash`.
