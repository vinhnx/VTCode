...
| `Tab`                                       | Accept the visible inline suggestion; on an empty idle composer, cycle top-level agents; otherwise queue the current input. | Plain input box only. Slash lists, pickers, queued input, active overlays, and visible suggestions keep priority over top-level agent cycling. |
...
- On an empty idle composer, `Tab` cycles top-level agents through discovered agent specs and then back to `agent:default`.
