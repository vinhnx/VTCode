...
| **Context Retrieval** | Retrieve offloaded info via search. | **Implicit**. Agent uses tools (`grep`, `read_file`) to retrieve info. `SessionArchive` exists but isn't actively queried for context logic. | **Medium**. Could implement "Recall" tool to search past conversation history from `SessionArchive`. |
