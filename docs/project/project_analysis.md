...
-   Workspace is a multi-crate Rust project (13 workspace members plus excluded security/support crates): CLI binary (`vtcode`) depends on `vtcode-core` and companion crates (`vtcode-ui`, `vtcode-config`, `vtcode-commons`, etc.), with membership declared in root `Cargo.toml`. F:Cargo.toml†L1-L120
