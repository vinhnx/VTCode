...
| `agent.harness.auto_compaction_enabled` | `boolean` | no | `true` | Enable automatic context compaction when token pressure crosses threshold. Enabled by default. When disabled, no automatic compaction is triggered. |
