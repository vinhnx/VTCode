# VTCode Terminal Optimization Guide

This guide covers the terminal optimization features available in VTCode, designed to enhance your terminal coding experience with advanced capabilities similar to Claude Code.
...
VTCode supports configurable themes for optimal visual experience. You can configure the theme in your `vtcode.toml` configuration file:
...
VTCode provides multiple options for handling line breaks in the terminal:
...
VTCode includes a Vim mode with a subset of Vim keybindings for efficient text editing:
...
VTCode provides comprehensive notification support for task completion and important events:
...
- VTCode supports terminal bell notifications for important events
...
VTCode provides several methods for handling large inputs efficiently:
...
### Piped Input
You can pipe content directly to VTCode:
```bash
cat large_file.txt | vtcode ask "Analyze this content"
```

### Large Output Handling
VTCode automatically handles large tool outputs by:
...
task_completion = [
  {
    ...
    hooks = [
      {
        command = "osascript -e 'display notification \"VTCode task completed\" with title \"VTCode\"' 2>/dev/null || true",
        timeout_seconds = 5
      }
    ]
  }
...
session_start = [
  {
    hooks = [
      {
        command = "echo 'VTCode session started at $(date)' >> ~/.vtcode/session.log",
        timeout_seconds = 5
      }
    ]
  }
...
This guide provides the foundation for optimizing your VTCode terminal experience. For more detailed information about specific features, refer to the relevant documentation sections.