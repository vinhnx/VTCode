# VTCode Allowed Commands Reference

This document outlines all commands that VTCode allows agents to execute, organized by category.

## Overview

VTCode maintains a comprehensive allow-list of safe commands that the agent can execute. The list is defined in `vtcode-config/src/constants.rs` and can be customized via `vtcode.toml`.
...
When executing commands, VTCode now preserves these critical environment variables from the parent shell:
...
**Note**: VTCode overrides `PAGER`, `GIT_PAGER`, `LESS`, `TERM`, color-related vars for consistency.
...
3. VTCode doesn't execute blocked commands - check against deny lists
