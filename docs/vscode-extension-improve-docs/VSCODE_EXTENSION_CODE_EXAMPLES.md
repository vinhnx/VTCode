  ...
  private async extractDependencies(): Promise<string> {
    ...
    for (const pattern of patterns) {
      ...
      if (file) {
        ...
        deps.push(`- **${file.fsPath.split("/").pop()}**:\n${doc.getText()}`)
      }
    }
...
export class ParticipantRegistry {
  ...
  async resolveMessage(
    ...
    for (const participant of this.getParticipants()) {
      if (participant.canHandle(context) && participant.resolveReferenceContext) {
        try {
          resolved = await participant.resolveReferenceContext(resolved, context)
        } catch (error) {
          console.error(
...
export class AskCommand implements ICommand {
  ...
  readonly description = "Send a question to the VTCode agent"
  ...
  async execute(context: CommandContext): Promise<void> {
    const question = await vscode.window.showInputBox({
      prompt: "What would you like VTCode to help with?",
    ...
    try {
      const promptWithContext = await this.appendIdeContext(question, context)
      ...
      void vscode.window.showInformationMessage(
        "VTCode finished processing your request. Check the output channel for details."
      )
    } catch (error) {
...
export class AskSelectionCommand implements ICommand {
  ...
  readonly description = "Ask VTCode about the selected code"
...
export class ToolApprovalManager implements vscode.Disposable {
  ...
  private onApprovalResponse = new vscode.EventEmitter<ToolApprovalResponse>()
      ...
      for (const [name, type] of files) {
        if (type === vscode.FileType.File && name.endsWith(".json")) {
          try {
            ...
            const thread = JSON.parse(Buffer.from(data).toString("utf8"))
...
export class ErrorRecoveryHandler {
  ...
  private async handleTokenLimit(context: ErrorContext): Promise<void> {
    ...
    if (messages.length > 5) {
      ...
      const summary = await context.backend.summarizeMessages(oldMessages)
      context.messages = [
...
async function activate(context: vscode.ExtensionContext) {
  ...
  const conversationManager = new ConversationManager(context.globalStorageUri)
...
describe("WorkspaceParticipant", () => {
  ...
  it("should resolve @workspace references", async () => {
    ...
    const resolved = await participant.resolveReferenceContext(message, context)
